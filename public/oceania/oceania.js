...
function search_bar() {
    ...
    oceaniaDiv = document.getElementById('oceania-list'); // pega a div em que os dados estao listados e coloca na variavel 'oceaniaDiv'
    ...
    for (i = 0; i < countryDiv.length; i++) { // ira verificar se o pais que foi digitado corresponde com algum, se sim, irá exibi-lo
        countryName = removeAccents(countryDiv[i].getElementsByClassName('nome_pais')[0].textContent || countryDiv[i].getElementsByClassName('nome_pais')[0].innerText); // será exibido o estilo de exibição da div do país atual como vazio.
